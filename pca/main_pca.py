...
def mnist17_01b_flip():
    ...
    poisoned_batch_mask_01 = np.zeros(shape=(NUM_TRAIN_BATCHES,), dtype=np.bool_)
    poisoned_batch_mask_025 = np.zeros(shape=(NUM_TRAIN_BATCHES,), dtype=np.bool_)
    poisoned_batch_mask_05 = np.zeros(shape=(NUM_TRAIN_BATCHES,), dtype=np.bool_)
    ...
    true_cluster_names = ['Unmodified', 'Flip proba = 0.1', 'Flip proba = 0.25', 'Flip proba = 0.5']
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        utils_common.plot_mmap_pca(mmap_pca, current_epoch, pred_cluster_masks, cluster_names, cluster_colours,
                                   explained_var, singular_values, 0, 1, model_dir, is_true=False)
        utils_common.plot_mmap_pca(mmap_pca, current_epoch, true_cluster_labels, true_cluster_names, true_cluster_colours,
                                   explained_var, singular_values, 0, 1, model_dir, is_true=True)
...
def mnist17_01c_noise():
    ...
    for noise_std, poisoned_batch_settings in noise_settings:
        for start_idx, end_idx in poisoned_batch_settings:
            train_x = utils_common._add_noise(train_x, start_idx, end_idx, noise_std, experiment_dir, plot=True)
