...
def _run_exp(params, shuffle_training_data, plot=False):
    ...
    # train_data, train_labels, test_data, test_labels = bridge_data_utils.get_data(
    #     './data/bridge3.csv', subtract=True)

    train_data, _, test_data, _ = synthetic_data.generate(params, plot=False, save=False)
    ...
    vae = vae_.VAE(input_dim=params.dim, shuffle_training_data=shuffle_training_data, save=True, mmap=True)
...
def _experiment_010(results_dir, shuffle_training_data):
    ...
    for id_, k in enumerate(ks):
        ...
        for i in range(MC_SAMPLES):
            ...
            _run_exp(params, shuffle_training_data, plot=i == MC_SAMPLES - 1)
...
def main():
    ...
    shuffle = False
    results_dir = os.path.join(OUT_DIR, 'seed{}_mmap_shuffle{}'.format(SEED, shuffle))
